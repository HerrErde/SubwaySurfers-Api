...
class tools:
    ...
    if platforn_name == "Darwin":
        tools_dir = (
            Path(home_path)
            .joinpath("Library", "Application Support", "apk-rebuild")
            .resolve()
        )
    ...
    bundletool_path = (
        Path(tools_dir)
        .joinpath(tools_data[0]["file_name"] + tools_data[0]["version"] + ".jar")
        .resolve()
    )
    apktool_path = (
        Path(tools_dir)
        .joinpath(tools_data[1]["file_name"] + tools_data[1]["version"] + ".jar")
        .resolve()
    )
    uber_apk_signer_path = (
        Path(tools_dir)
        .joinpath(tools_data[2]["file_name"] + tools_data[2]["version"] + ".jar")
        .resolve()
    )
...
def check_tools():
    ...
    for tool in tools.tools_data:
        tool_file = (
            Path(tools.tools_dir)
            .joinpath(tool["file_name"] + tool["version"] + ".jar")
            .resolve()
        )
        tool_url = (
            tool["url"]
    ...
    if not args.ks:
        ...
        debug_keystore_full_path = (
            Path(debug_keystore_path).joinpath("debug.keystore").resolve()
        )
        if not debug_keystore_full_path.exists():
            debug_keystore_source_full_path = (
                Path(sys.argv[0]).parent.joinpath("debug.keystore").resolve()
            )
            log_warn(
                f"File {debug_keystore_full_path} not found, copying it from {debug_keystore_source_full_path}"
            )
        ...
        if not Path(args.ks).resolve().exists():
            ...
            if not args.ks_pass or not args.ks_alias or not args.ks_alias_pass:
                if not args.ks_pass:
                    log_err(
                        f"Keystore password is missing, specify it with --ks-pass argument"
                    )
                    have_all_tools = False
                if not args.ks_alias:
                    log_err(
                        f"Keystore alias is missing, specify it with --ks-alias argument"
                    )
                    have_all_tools = False
                if not args.ks_alias_pass:
                    log_err(
                        f"Key password is missing, specify it with --ks-key-pass argument"
                    )
                    have_all_tools = False
            else:
                command_output = subprocess.run(
...
def rebuild_single_apk(arg_source_apk_full_path, arg_output_apk_full_path):
    ...
    if decompiled_path.exists():
        log_err(
            f"Directory {decompiled_path} already exists, probably the .apk file {arg_source_apk_full_path} was already processed by the script. Remove the directory and run the script again. Stopping the script"
        )
        exit_script(1)
    ...
    command = [
        "java",
        "-jar",
        str(tools.apktool_path),
        "decode",
        str(arg_source_apk_full_path),
        "--output",
        str(decompiled_path),
    ]
    ...
    if args.apihost_patch:
        # edit api host
        fpath = (
            decompiled_path
            / "assets"
            / "settings"
            / "sybo.network.connection.listener.json"
        )
        if not fpath.exists():
        ...
        with fpath.open("r+", encoding="utf-8") as f:
            data = json.load(f)
            if "PingHost" not in data:
                raise KeyError('"PingHost" key not found in JSON.')
            data["PingHost"] = args.apihost_patch
            f.seek(0), json.dump(data, f, indent=2), f.truncate()

    if args.network_patch:
        ...
        network_security_config_full_path = network_security_config_path.joinpath(
        ...
        if not network_security_config_full_path.exists():
            ...
            etree.ElementTree(root).write(
                str(network_security_config_full_path),
                pretty_print=True,
                encoding="utf-8",
            ...
            if tree.find("base-config/trust-anchors/certificates[@src='user']") == None:
                log_warn(
                    "File /res/xml/network_security_config.xml doesn't meet the requirements"
                )
                root = tree.getroot()
                ...
                if elem_base == None:
                    log_info(
                        'Adding element <base-config cleartextTrafficPermitted="false">'
                    )
                    root.append(
                        etree.Element("base-config", cleartextTrafficPermitted="false")
                    )
                    elem_base = tree.find("base-config")
                ...
                for ca_type in ["system", "user"]:
                    if (
                        tree.find(
                            "base-config/trust-anchors/certificates[@src='"
                            + ca_type
                            + "']"
                        )
                        == None
                ...
                log_succ(
                    "File /res/xml/network_security_config.xml meets the requirements"
                )

        # processing AndroidManifest.xml
        android_manifest_full_path = decompiled_path.joinpath(
            "AndroidManifest.xml"
        ).resolve()
        ...
        if (
            ...
            log_info(
                'Adding attribute android:networkSecurityConfig="@xml/network_security_config"'
            )
            # updating 'application' tag
            elem_application.set(
                "{http://schemas.android.com/apk/res/android}networkSecurityConfig",
                "@xml/network_security_config",
            )
            # writing AndroidManifest.xml
            etree.ElementTree(root).write(
                str(android_manifest_full_path),
                pretty_print=True,
                encoding="utf-8",
                xml_declaration=True,
            )
        ...
        log_info("Skipping network config patching, use --patch argument to enable it")
    ...
    command = [
        "java",
        "-jar",
        str(tools.uber_apk_signer_path),
        "--apks",
        str(arg_output_apk_full_path),
        "--allowResign",
        "--overwrite",
...
def main():
    ...
    parser.add_argument(
        "source_file",
        metavar="file",
        help="path to .apk, .aab or .xapk file for rebuilding",
    )
    parser.add_argument(
        "-v", "--version", action="version", version=f"%(prog)s {script_version}"
    )
    parser.add_argument(
        "-i",
        "--install",
        action="store_true",
        help="install the rebuilded .apk file(s) via adb",
    )
    parser.add_argument(
        "--pause",
        action="store_true",
        help="pause the script execution before the building the output .apk",
    )
    parser.add_argument(
        "-p",
        "--preserve",
        action="store_true",
        help="preserve the unpacked content of the .apk file(s)",
    )
    parser.add_argument(
        "-r",
        "--remove",
        action="store_true",
        help="remove the source file (.apk, .aab or .xapk) after the rebuilding",
    )
    parser.add_argument(
        "-o",
        "--output",
        help="output .apk file name or output directory path (for .xapk source file)",
    )
    parser.add_argument(
        "--no-src",
        action="store_const",
        const="--no-src",
        help="use --no-src option when decompiling via apktool",
    )
    parser.add_argument(
        "--only-main-classes",
        action="store_const",
        const="--only-main-classes",
        help="use --only-main-classes option when decompiling via apktool",
    )
    parser.add_argument(
        "--ks", help="use custom .keystore file for .aab decoding and .apk signing"
    )
    ...
    parser.add_argument(
        "--ks-alias-pass", help="password for key (alias) in the custom keystore"
    )
    parser.add_argument(
        "-net",
        "--network-patch",
        action="store_true",
        help="patch network config",
    )
    parser.add_argument(
        "-api",
        "--apihost-patch",
        help="change the apihost",
    )
    ...
    if not Path(args.source_file).resolve().exists():
        log_err(
            f"File {Path(args.source_file).resolve()} not found, stopping the script"
        )
        exit_script(1)
    ...
    if args.output != None:
        if source_file.ext.lower() == ".xapk":
            ...
            Path(output_files.directory_path).mkdir(parents=True, exist_ok=True)
            output_files.full_path = Path(
                str(Path(args.output).resolve()) + output_suffix + output_ext
            ).resolve()
    else:
        if source_file.ext.lower() == ".xapk":
            output_files.directory_path = Path(source_file.directory_path).joinpath(
                source_file.name_wo_ext
            )
        else:
            output_files.directory_path = source_file.directory_path
            output_files.full_path = Path(
                str(source_file.full_path_wo_ext) + output_suffix + output_ext
            ).resolve()
    ...
    if source_file.ext.lower() == ".apk":
        ...
        command = [
            "java",
            "-jar",
            str(tools.bundletool_path),
            "build-apks",
            "--bundle=" + str(source_file.full_path),
            "--output=" + str(apks_full_path),
            "--mode=universal",
        ]
        # execute bundletool
        subprocess.run(command, stdout=sys.stdout, stderr=sys.stderr)
        ...
        apks_dir_full_path = source_file.directory_path.joinpath(
            source_file.name_wo_ext
        ).resolve()
        ...
        shutil.move(
            apks_dir_full_path.joinpath("universal.apk").resolve(), apk_full_path
        )
        ...
        for single_apk in apk_files_list:
            single_apk_new = xapk_dir_full_path.joinpath(
                str(Path(single_apk.name).with_suffix("")) + output_suffix + output_ext
            ).resolve()
    ...
    if args.install:
        ...
        if "debug" in command_output.lower():
            if source_file.ext.lower() == ".xapk":
                ...
                log_info(
                    f"Installing the rebuilded .apk file {colors.WARNING}{output_files.full_path}"
                )
    ...
    if source_file.ext.lower() == ".xapk":
        ...
        log_info(
            f"Command for installing: {colors.OKGREEN}adb install-multiple {str_apk_list}"
        )
    else:
        log_info(
            f'Command for installing: {colors.OKGREEN}adb install "{str(output_files.full_path)}"'
        )
